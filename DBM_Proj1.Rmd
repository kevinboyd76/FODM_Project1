...
+ **tried to graph but having issue...need help**
...
developedVSdeveloping$percent=((developedVSdeveloping$Developed/developedVSdeveloping$Developing)*100)

developedVSdeveloping

###help here can't seem to graph thisss...
#developedVSdeveloping %>% ggplot(aes(x=percent, fill=disease)) + geom_bar(stat="identity")
...
CountryCasesStatus %>% ggplot(aes(x=ActualCases, y=disease,color=disease)) + geom_boxplot()
allCases %>% ggplot(aes(x=cases, fill=disease)) + geom_histogram() + facet_wrap(~disease, ncol=1)
ggplot(allCases, aes(y=cases, x=disease, color=Status.x)) + geom_boxplot()+ facet_wrap(~Status.x, ncol=1)
