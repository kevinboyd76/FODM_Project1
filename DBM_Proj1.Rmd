...
+ 133 countries without a specific disease
+ 114 Polio and 19 Measles

Countries without measles cases
Status
       n  missing 
      19        0        
...
Frequency           1         18
Proportion      0.053      0.947


Countries without polio cases
Status.x 
       n  missing  
     114        0        
...
+ Now when it come to actual cases in developed vs developing
+ **Here is the table of tha actual difference and percentage which is actually not that much**
...
developedVSdeveloping=CountryCasesStatus %>% select(disease, Status.x, ActualCases) %>% spread( Status.x,ActualCases)
developedVSdeveloping$percentDifference=round(((developedVSdeveloping$Developed/developedVSdeveloping$Developing)*100),2)

developedVSdeveloping

library(ggplot2)
###help here can't seem to graph thisss...
...
CountryCasesStatus %>% ggplot(aes(x=ActualCases, y=disease,color=disease)) + geom_boxplot()+scale_fill_brewer(palette = "Set2")+ theme_classic() 
...
ggplot(allCases, aes(y=cases, x=disease, color=disease)) + geom_boxplot()+ facet_wrap(~Status.x, ncol=1)+scale_fill_brewer(palette = "Set2")+ theme_classic() 
...
geom_text(data=perAllCases, aes(x=Status.x, label=paste(percentWithDisease,"%"), y=percent, fill=NULL), nudge_y=5) + theme_classic() + ggtitle("Pecent of developed and developing with diseases ")+ xlab("Status")+ scale_fill_brewer(palette = "Set2")
```
...
geom_text(data=tuberPerc, aes(x=Status, label=paste(percent,"%"), y=percent, fill=NULL), nudge_y=5) +
  scale_fill_brewer(palette = "Set2")+ theme_classic() + ggtitle("Pecent status Tuberculosis Data ")

b=hpbPerc %>% ggplot(aes(x=Status, y=percent, fill=Status)) + geom_col()+
geom_text(data=tuberPerc, aes(x=Status, label=paste(percent,"%"), y=percent, fill=NULL), nudge_y=5) +
  scale_fill_brewer(palette = "Set2")+ theme_classic() + ggtitle("Pecent status HPB2 Data ")


c=polioPerc %>% ggplot(aes(x=Status, y=percent, fill=Status)) + geom_col()+
geom_text(data=tuberPerc, aes(x=Status, label=paste(percent,"%"), y=percent, fill=NULL), nudge_y=5) +
  scale_fill_brewer(palette = "Set2")+ theme_classic() + ggtitle("Pecent status in Polio Data ")
